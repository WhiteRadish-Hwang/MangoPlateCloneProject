...
class GangbukFragment : BaseFragment<FragmentLocSelectGangbukBinding>(FragmentLocSelectGangbukBinding::bind, R.layout.fragment_loc_select_gangbuk){
    companion object {
        var isLocCheck: Boolean = false
        var locList = ArrayList<String>()
    }

    lateinit var locSelectRecyclerAdapter: LocSelectRecyclerAdapter
    ...
    fun setRecyclerAdapter(){
        initData()
        locSelectRecyclerAdapter = LocSelectRecyclerAdapter(context, itemList)
        binding.gangbukRecycler.apply {
            layoutManager = GridLayoutManager(context,2, LinearLayoutManager.VERTICAL, false)
            setHasFixedSize(true)
            //아이템 클릭 리스너
            locSelectRecyclerAdapter.let {
                it.setMyLocItemClickListener(object : LocSelectRecyclerAdapter.MyLocItemClickListener {
                    override fun onItemClick(position: Int) {
                        showCustomToast("${itemList[position].location}")

                    }
                })
            }
            adapter = locSelectRecyclerAdapter
        }
